...
Windows 会产出 `.exe` 安装包和 `.msi`，但现在还没有做真实 Windows 环境下的完整功能验证，先当实验性版本看。

## 平台状态

macOS 是目前主要验证过的平台。状态栏常驻、Provider 切换、Codex 配置写入、线程 Provider 合并，这些都已经在 macOS 上实际跑过。

Windows 目前只确认了 CI 可以编译出安装包。还没有在真实 Windows 机器上完整验证这些行为：

- Provider 切换后 `config.toml` / `auth.json` 是否按预期写入。
- `CODEX_HOME` 和默认 `.codex` 路径在不同 Windows 用户环境下是否都正常。
- 线程 Provider 合并是否和 macOS 行为一致。
- 托盘菜单、窗口显示、退出行为是否符合 Windows 用户习惯。

所以 Windows 版本现在更适合测试，不建议直接用于重要的 Codex 工作目录。
